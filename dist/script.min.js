const modal=document.querySelector(".modal"),span=modal.querySelector("span"),content=modal.querySelector(".modal-content"),nav=document.querySelector(".main-navigation"),download=document.querySelector(".download > a");let clone,images,showIndex=0;function showModal(e,n,o){images=e,showIndex=o,modal.style.display="block",clone=n.cloneNode(!0),content.appendChild(clone),updateText(),nav.style.display="none",download.setAttribute("href",addLink(n))}function closeModal(){content.removeChild(clone),modal.style.display="none",nav.style.display="block"}function nextImage(){showImage(1),updateText()}function prevImage(){showImage(-1),updateText()}function showImage(e){content.removeChild(clone),setIndex(e),clone=images[showIndex].cloneNode(!0),download.setAttribute("href",addLink(clone)),content.appendChild(clone)}function setIndex(e){showIndex+=e,-1===showIndex&&(showIndex=images.length-1),showIndex===images.length&&(showIndex=0)}function updateText(){span.innerHTML=`${showIndex+1} / ${images.length}`}function addLink(e){return console.log(e.getAttribute("src")),e.getAttribute("src")}
/*! lazysizes - v5.2.0 */
...
(()=>{const n=document.querySelector(".js-main_navigation_mobile-button"),e=document.querySelector(".js-main-navigation_menu"),t=()=>{e.classList.toggle("main-navigation_menu--open")};n.addEventListener("click",t),e.addEventListener("click",t)})();